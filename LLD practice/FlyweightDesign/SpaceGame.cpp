...
//we can refer the flyweight object hwere we need to save the ram memory[by diving object creation
//between instrinsic and extrinsic properties so that we can reuse a object if it has created with similar properties]
class AsteriodFlyWieght{
    private:
        int length;
        int width;
        int weight;
        ...
        string material;
    public:
        AsteriodFlyWieght(int l, int w, int wt, string col, string txt, string mat){
            this->length = l;
            this->width = w;
            this->weight = w;
            this->color = col;
        ...
        //extrinsic properties of each object we will set dynamically
...
class AsteriodFactory{
    private:
        static unordered_map<string, AsteriodFlyWieght*> flyweights;

    public:
        static AsteriodFlyWieght* getAsteroid(int length, int width, int weight, 
        ...
        if (flyweights.find(key) == flyweights.end()) {
            flyweights[key] = new AsteriodFlyWieght(length, width, weight, color, texture, material);
        }
    ...
    static size_t getTotalFlyweightMemory() {
        return flyweights.size() * AsteriodFlyWieght::getMemoryUsage();
    }
...
unordered_map<string, AsteriodFlyWieght*> AsteriodFactory::flyweights;


// Context - Stores EXTRINSIC state only
class AsteroidContext {
private:
    AsteriodFlyWieght* flyweight;
    int posX, posY;                        // 8 bytes (position)
    int velocityX, velocityY;              // 8 bytes (velocity)
    
public:
    
AsteroidContext(AsteriodFlyWieght* fw, int posX, int posY, int velX, int velY) {
        this->flyweight = fw;
        this->posX = posX;
        this->posY = posY;
        this->velocityX = velX;
    ...
    static size_t getMemoryUsage() {
        return sizeof(AsteriodFlyWieght*) + 
                sizeof(int) * 4;                
    }
};
...
public:
    void spawnAsteroids(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            AsteriodFlyWieght* flyweight = AsteriodFactory::getAsteroid(
        ...
        cout << "Total flyweight objects: " << AsteriodFactory::getFlyweightCount() << endl;
    }
    ...
    size_t calculateMemoryUsage() {
        ...
        size_t flyweightMemory = AsteriodFactory::getTotalFlyweightMemory();
